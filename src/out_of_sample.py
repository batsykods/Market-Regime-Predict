...
def main():
    ...
    dev_risk = development["Predicted_Return"] / (
    ...
    test["Net_Return"] = test["Strategy_Return"] - test["Transaction_Cost"]
    ...
    # Trade-level statistics use only active trading days, not flat days.
    active_returns = r[test["Position"] > 0]
    ...
    trade_win_rate = (active_returns > 0).mean() if len(active_returns) else 0.0
    ...
    print(f"CAGR: {cagr:.4%}")
    print(f"Annualised Volatility: {ann_vol:.4%}")
    ...
    print(f"Trading-Day Win Rate: {trade_win_rate:.2%}")
    print(f"Profit Factor: {pf:.4f}")
    print(f"Average Active-Day Win: {wins.mean() if len(wins) else 0:.6f}")
    print(f"Average Active-Day Loss: {losses.mean() if len(losses) else 0:.6f}")
    ...
    for regime, g in test.groupby("Regime"):
        ...
        signal_returns = g.loc[g["Signal"], "Future_Return"]
        print(
            f"Regime {int(regime)}: samples={len(g)}, "
            f"mean future return={g['Future_Return'].mean():.4%}, "
            f"direction accuracy={acc:.2%}, "
            f"signals={int(g['Signal'].sum())}, "
            f"signal mean return={signal_returns.mean() if len(signal_returns) else 0:.4%}"
        )
